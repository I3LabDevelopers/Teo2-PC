        ...
        private void XBInputHandler()
        {
            ...
            while (true)
            {
                if (xbc.IsConnected)
                {
                    ...
                    if (xbc.ButtonBackPressed && !last_BK)
                    {
                        input_buffer.Push("BACK");
                        OnXBInputReceived(EventArgs.Empty);

                    }
                    last_BK = xbc.ButtonBackPressed;

                    if (xbc.ButtonStartPressed && !last_ST)
                    {
                        input_buffer.Push("START");
                        OnXBInputReceived(EventArgs.Empty);

                    }
                    last_ST = xbc.ButtonStartPressed;
